...
func _start_level(level_index: int) -> void:
	...
	hud.show()
	hud.update_display(current_level.get_stage_name(), "00:00.00", 0)
	pause_menu.hide()
	get_tree().paused = false


func _on_hud_updated(stage_name: String, time_text: String, deaths: int) -> void:
	hud.update_display(stage_name, time_text, deaths)
...
func _ensure_overlay_scenes() -> void:
	...
	hud.hide()
	hud.climb_pressed.connect(func() -> void:
		if current_player != null and not get_tree().paused:
	...
	pause_menu.title_pressed.connect(func() -> void:
		_resume_game()
		GameManager.show_title()
	)
